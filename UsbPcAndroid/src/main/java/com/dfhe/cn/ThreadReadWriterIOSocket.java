...
import com.dfhe.cn.util.FileHelper;
import com.dfhe.cn.util.MyUtil;
...
public class ThreadReadWriterIOSocket implements Runnable {
	...
	@Override
	public void run() {
		Log.e(AndroidService_.TAG, Thread.currentThread().getName() + "---->"
		...
		try {
			...
			AndroidService_.ioThreadFlag = true;
			while (AndroidService_.ioThreadFlag) {
				try {
			...
			try {
				if (client != null) {
					Log.v(AndroidService_.TAG, Thread.currentThread().getName()
							+ "---->" + "client.close()");
					client.close();
				}
			} catch (IOException e) {
